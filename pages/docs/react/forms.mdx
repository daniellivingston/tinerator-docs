import withDocs from '../../../components/with_docs';

export default withDocs({
  title: 'Forms',
  description: 'How to use the StaticKit form component.',
  path: '/docs/react/forms',
  breadcrumb: {
    path: '/docs/react',
    label: 'React Library',
    rel: 'prev'
  },
  toc: [
    [1, "Simple example", "#simple-example"],
    [1, "State", "#state"],
    [1, "Handling errors", "#handling-errors"],
    [1, "Configuration", "#configuration"]
  ],
  prev: {
    path: '/docs/react',
    label: 'Installation'
  },
});

# Forms

React forms are powered by a custom [hook](https://reactjs.org/docs/hooks-intro.html) called `useForm`. 

The `useForm` hook returns a pair of values: a `state` object containing the current state of the form and a `submit` function that performs the actual form submission (and updates the `state` accordingly).

```js
const [state, submit] = useForm({
  site: 'XXXXXXXXX',
  form: 'contact-form'
});
```

## Simple example

Here's a form component built with StaticKit:

```jsx
import { useForm } from '@statickit/react';

function MyForm() {
  // Call the `useForm` hook in your function component
  // Replace 'XXXXXXXXX' with your StaticKit site id
  const [state, submit] = useForm({
    site: 'XXXXXXXXX',
    form: 'contact-form'
  });

  // Display success message in place of the form
  if (state.succeeded) {
    return (
      <div>Thank you for signing up!</div>
    )
  }

  return (
    <form onSubmit={submit}>
      <label htmlFor="email">Email</label>
      <input id="email" type="email" name="email" />
      <button type="submit">Sign up</button>
    </form>
  )
}
```

At a minimum, all you have to do is use a `<form>` element and pass `submit` as the `onSubmit` handler.

Here is a working example of an opt-in form. Try it out!

<div className="pb-6">
  <iframe width="100%" height="300" className="rounded-lg" src="//jsfiddle.net/StaticKit/w69xerbc/embedded/result,js,html/dark/" allowFullScreen="allowfullscreen" frameBorder="0"></iframe>
</div>

## State

The `state` object contains the following:

| Key            | Description                                                                         |
| :------------- | :---------------------------------------------------------------------------------- |
| `submitting`   | A Boolean indicating whether the form is currently submitting (defaults to `false`) |
| `succeeded`    | A Boolean indicating whether the form successfully submitted (defaults to `false`)  |
| `errors`       | An Array of server-side validation errors (defaults to `[]`)                        |

Items in the `errors` array contain the following:

| Key            | Description                                                       |
| :------------- | :---------------------------------------------------------------- |
| `field`        | The name of the field                                             |
| `message`      | A human-readable error message fragment (e.g. "is required")      |
| `code`         | A machine-friendly error code (e.g. "REQUIRED" or "EMAIL_FORMAT") |

### Lifecycle

- When the user submits the form, `submitting` becomes `true`
- If the submission fails server-side validations:
  - `succeeded` remains `false`, and
  - `errors` is populated with the specific errors
- Otherwise, `succeeded` becomes `true`

## Handling errors

Here's a more advanced example that displays validation errors for the `email` field:

```diff
- import { useForm } from '@statickit/react';
+ import { ValidationError, useForm } from '@statickit/react';

  function MyForm() {
    const [state, submit] = useForm('XXXXXXXXX');

    if (state.succeeded) {
      return (
        <div>Thank you for signing up!</div>
      )
    }

+   // Render email validation errors and disable the submit button when submitting
    return (
      <form onSubmit={submit}>
        <label htmlFor="email">Email</label>
        <input type="email" name="email" required />
+       <ValidationError field="email" prefix="Email" errors={state.errors} />
        <button type="submit" disabled={state.submitting}>Sign up</button>
      </form>
    )
  }
```

The `ValidationError` component accepts the following special properties:

- `field` — the name of the field for which to display errors (required)
- `errors` — the object containing validation errors (required)
- `prefix` — the human-friendly name of the field (optional, defaults to `This field`)

If the specified field has a validation error, this component renders a `<div>` containing the error message:

```html
<div>
  {prefix} {message}
</div>
```

If the field does not have an error, nothing is rendered.

All other props set on `ValidationError` (such as `className`) are passed through to the `<div>` wrapper.

## Configuration

The `useForm` hook accepts the following properties:

| Key                                | Type                | Description                                           |
| :--------------------------------- | :------------------ | :---------------------------------------------------- |
| [`id`](#id)                        | String              | The form ID                                           |
| [`site`](#site)                    | String              | The site ID                                           |
| [`form`](#form)                    | String              | The form key                                          |
| [`data`](#data)                    | Object              | An object containing additional form fields to send   |

### `id`

- **Type**: String
- **Description**: The form ID
- **Example Value**: `f53fe2faa75d`

The property identifies which form to associate submissions with on the server-side.

```js
const [state, submit] = useForm({
  id: 'f53fe2faa75d'
});
```

### `site`

- **Type**: String
- **Description**: The site ID
- **Example Value**: `a38ad7363b35`

Use this property in conjunction with the `form` property to identify the server-side form.

### `form`

- **Type**: String
- **Description**: The form key
- **Example Value**: `newsletter`

Use this property in conjunction with the `site` property to identify the server-side form.

```js
// In your statickit.json file
{
  "forms": {
    "newsletter": {
      "name": "Newsletter"
    }
  }
}

// In your component
const [state, submit] = useForm({
  site: 'a38ad7363b35',
  form: 'newsletter'
});
```

### `data`

- **Type**: Object containing Strings or Functions
- **Description**: Additional data to include with the form submission.

```js
const [state, submit] = useForm({
  site: 'a38ad7363b35',
  form: 'newsletter',
  data: {
    _subject: 'Someone joined the newsletter',
    pageTitle: function() {
      // This function will be evaluated at submission time
      return document.title;
    }
  }
});
```
